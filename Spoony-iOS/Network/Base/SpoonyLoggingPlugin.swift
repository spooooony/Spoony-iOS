...
public final class SpoonyLoggingPlugin: PluginType {
    ...
    public init(verbose: Bool = true, cURL: Bool = false, output: ((_ separator: String, _ terminator: String, _ items: Any...) -> Void)? = nil, requestDataFormatter: ((Data) -> (String))? = nil, responseDataFormatter: ((Data) -> (Data))? = nil) {
        ...
        self.output = output ?? SpoonyLoggingPlugin.reversedPrint
...
private extension SpoonyLoggingPlugin {
    ...
    func logNetworkRequest(_ request: URLRequest?) -> [String] {
        ...
        output += [format(loggerId, date: date, identifier: "💬 RequestURL", message: request?.description ?? "(invalid request)")]
        ...
        if let bodyStream = request?.httpBodyStream {
            output += [format(loggerId, date: date, identifier: "Request Body  ", message: bodyStream.description)]
        }
...
fileprivate extension SpoonyLoggingPlugin {
